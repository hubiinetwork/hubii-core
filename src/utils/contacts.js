import { IsAddressMatch } from './wallet';
const MAX_RECENT_CONTACTS_LENGTH = 5;

export function updateRecentContacts(allContacts, recentContacts, transaction) {
  const index = allContacts.findIndex((contact) => IsAddressMatch(contact.address, transaction.to));
  if (index < 0) {
    return recentContacts;
  }
  const newRecentContact = allContacts[index];
  const updatedRecentContacts = recentContacts.filter((contact) => !IsAddressMatch(contact.address, transaction.to));
