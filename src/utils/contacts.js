...
export function updateRecentContacts(allContacts, recentContacts, transaction) {
  const index = allContacts.findIndex((contact) => contact.address === transaction.to.toLowerCase());
  if (index < 0) {
    return recentContacts;
  }
  const newRecentContact = allContacts[index];
  const updatedRecentContacts = recentContacts.filter((contact) => contact.address !== transaction.to.toLowerCase());
