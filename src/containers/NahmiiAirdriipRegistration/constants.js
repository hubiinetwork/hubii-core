/*
 *
 * NahmiiAirdriipRegistration constants
 *
 */

export const CHANGE_STAGE = 'src/NahmiiAirdriipRegistration/CHANGE_STAGE';
export const CHANGE_SELECTED_CORE_WALLET = 'src/NahmiiAirdriipRegistration/CHANGE_SELECTED_CORE_WALLET';
export const CHANGE_MANUAL_SIGNED_MESSAGE = 'src/NahmiiAirdriipRegistration/CHANGE_MANUAL_SIGNED_MESSAGE';
export const CHANGE_MANUAL_ADDRESS = 'src/NahmiiAirdriipRegistration/CHANGE_MANUAL_ADDRESS';
export const REGISTER = 'src/NahmiiAirdriipRegistration/REGISTER';
export const REGISTRATION_FAILED = 'src/NahmiiAirdriipRegistration/REGISTRATION_FAILED';
export const REGISTRATION_SUCCESS = 'src/NahmiiAirdriipRegistration/REGISTRATION_SUCCESS';
