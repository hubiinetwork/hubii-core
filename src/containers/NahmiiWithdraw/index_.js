...
export class NahmiiWithdraw extends React.PureComponent {
  constructor(props) {
    ...
    this.state = { amountToStageInput: '0', amountToWithdrawInput: '0', amountToWithdraw: new BigNumber(0), syncTime: new Date(), selectedSymbol: 'ETH', expandedTxs: new Set() };
  ...
  getRequiredSettlementAmount(withdrawalAmountBN) {
    const {staged} = this.props;
    if (!staged) {
      return null;
    }
    const stagedAsset = staged.assets.find((a) => a.symbol === selectedSymbol) || { balance: new BigNumber('0') };
    if (withdrawalAmountBN > stagedAsset.balance) {
      return withdrawalAmountBN.sub(stagedAsset.balance);
    }
    return null
  }
  ...
  render() {
    ...
    const baseLayerAsset = baseLayerAssets.find((a) => a.symbol === selectedSymbol);

    if (!selectedAssetDetails || !staged || !available || !baseLayerAsset) {
      return (null);
    }
    ...
    availableAsset.balance = new BigNumber(availableAsset.balance)
    ...
    // const withdrawalAmountBN = ethersUtils.parseUnits(amountToWithdraw, decimals);
    const requiredSettlementAmountBN = this.getRequiredSettlementAmount(amountToWithdraw);
      ...
      <OuterWrapper>
        <WithdrawalWrapper>
          ...
          <WithdrawalDescriptionWrapper>
              ...
              <div>
                ...
                <Row>
                  <TransferDescriptionItem
                    main={`${ethersUtils.formatUnits(requiredSettlementAmountBN, decimals)} ${selectedSymbol}`}
                    subtitle={formatFiat(1, 'USD')}
                  />
                </Row>
                <Row>
                  <StyledButton onClick={this.startChallenge}>
                    {formatMessage({ id: 'settle_payment' })}
                  </StyledButton>
                </Row>
              </div>
            ...
            {
              requiredSettlementAmountBN === null &&
              <div>
                <Row>
                  <StyledCol span={12}>{formatMessage({ id: 'nahmii_balance_before' })}</StyledCol>
                </Row>
                <Row>
                  <TransferDescriptionItem
                    main={`${availableAsset.balance} ${selectedSymbol}`}
                    subtitle={formatFiat(1, 'USD')}
                  />
                </Row>
                <Row>
                  <StyledCol span={12}>{formatMessage({ id: 'nahmii_balance_after' })}</StyledCol>
                </Row>
                <Row>
                  <TransferDescriptionItem
                    main={`${availableAsset.balance.minus(amountToWithdraw)} ${selectedSymbol}`}
                    subtitle={formatFiat(1, 'USD')}
                  />
                </Row>
                <Row>
                  <StyledCol span={12}>{formatMessage({ id: 'base_layer' })} {selectedSymbol} {formatMessage({ id: 'balance_before' })}</StyledCol>
                </Row>
                <Row>
                  <TransferDescriptionItem
                    main={`${baseLayerAsset.balance} ${selectedSymbol}`}
                    subtitle={formatFiat(1, 'USD')}
                  />
                </Row>
                <Row>
                  <StyledCol span={12}>{formatMessage({ id: 'base_layer' })} {selectedSymbol} {formatMessage({ id: 'balance_after' })}</StyledCol>
                </Row>
                <Row>
                  <TransferDescriptionItem
                    main={`${baseLayerAsset.balance.add(amountToWithdraw)} ${selectedSymbol}`}
                    subtitle={formatFiat(1, 'USD')}
                  />
                </Row>
                <Row>
